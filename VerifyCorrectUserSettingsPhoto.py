...
class MyTestCase(unittest.TestCase):
    ...
    def test_verify_changing_photo(self):
        driver = self.driver
        change_photo = driver.find_element_by_xpath("//div[@class = 'MuiExpansionPanelSummary-content']").click()
        clear_button = driver.find_element_by_xpath("//*[@id='panel1bh-content']/div/p/form/button").click()
        Imagepath = r"..\seleniumEventExpress\avatar.png"
        change_photo_input = driver.find_element_by_xpath("//div[@class = 'placeholder-preview ']/input")
        change_photo_input.send_keys(os.path.abspath(Imagepath))
        submit_button = driver.find_element_by_xpath("// *[ @ id = 'panel1bh-content'] / div / p / form / div[2] / "
                                                     "button").click()
        success = driver.find_element_by_id("client-snackbar").text
        print(success)
        self.assertEqual("Avatar is update", success)
